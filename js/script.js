...
// Menghitung BMI
document.querySelector('.bg-green-yellow.color-black').addEventListener('click', function(e) {
    e.preventDefault();

    // Buat nilai input
    const berat = parseFloat(document.getElementById('input-berat-badan').value);
    const tinggi = parseFloat(document.getElementById('input-tinggi-badan').value);
    const usia = parseInt(document.getElementById('input-usia').value);

    // Validasi supaya nilai input adalah angka valid
    if (isNaN(berat) || isNaN(tinggi) || isNaN(usia) || tinggi<=0 || berat<=0) {
    ...
    document.getElementById('info-result').textContent = `Artinya: ${kategori}.`;
});


// Fungsi tombol reset
document.querySelector('.bg-alice-blue').addEventListener('click', function(e) {
    e.preventDefault();
